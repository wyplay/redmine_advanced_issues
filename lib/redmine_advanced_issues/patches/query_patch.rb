		...
		module QueryPatch
			...
			if Rails::VERSION::MAJOR < 3
					...
					returning available_columns_without_spent_hours do |columns|
						...
						if (project and User.current.allowed_to?(:view_time_entries, project)) or User.current.admin?
			      		...
			      		:caption => :label_calculated_spent_hours, 
								:sortable => "(IF(#{Issue.table_name}.estimated_hours IS NULL,0,#{Issue.table_name}.estimated_hours) * #{Issue.table_name}.done_ratio / 100.0))"
							...
							columns << QueryColumn.new(:calculated_spent_time, 
								...
								:sortable => "(IF(#{Issue.table_name}.estimated_hours IS NULL,0,#{Issue.table_name}.estimated_hours) * #{Issue.table_name}.done_ratio / 100.0)) / #{TimeManagement.getCoef(Setting.plugin_redmine_advanced_issues['default_unit'].to_s)}"
							) unless columns.detect { |c| c.name == :calculated_spent_time }

							columns << QueryColumn.new(:divergent_hours, 
								:caption => :label_divergent_hours, 
								:sortable => "((select sum(hours) from #{TimeEntry.table_name} where #{TimeEntry.table_name}.issue_id = #{Issue.table_name}.id) - (IF(#{Issue.table_name}.estimated_hours IS NULL,0,#{Issue.table_name}.estimated_hours) * #{Issue.table_name}.done_ratio / 100.0))"
							...
							columns << QueryColumn.new(:remaining_hours, 
								...
								:sortable => "(IF(#{Issue.table_name}.estimated_hours IS NULL,0,#{Issue.table_name}.estimated_hours) - (IF(#{Issue.table_name}.estimated_hours IS NULL,0,#{Issue.table_name}.estimated_hours) * #{Issue.table_name}.done_ratio / 100.0))"
							) unless columns.detect { |c| c.name == :remaining_hours }

							columns << QueryColumn.new(:remaining_time, 
								:caption => :label_remaining_time, 
								:sortable => "(IF(#{Issue.table_name}.estimated_hours IS NULL,0,#{Issue.table_name}.estimated_hours) - (IF(#{Issue.table_name}.estimated_hours IS NULL,0,#{Issue.table_name}.estimated_hours) * #{Issue.table_name}.done_ratio / 100.0)) / #{TimeManagement.getCoef(Setting.plugin_redmine_advanced_issues['default_unit'].to_s)}"
				...
				def available_columns_with_spent_hours
					...
					returning available_columns_without_spent_hours.tap do |columns|
						...
						if (project and User.current.allowed_to?(:view_time_entries, project)) or User.current.admin?
							...
							columns << QueryColumn.new(:spent_time, 
								:caption => :label_spent_time, 
								:sortable => "(select sum(hours) / #{TimeManagemprojectent.getCoef(Setting.plugin_redmine_advanced_issues['default_unit'].to_s)} from #{TimeEntry.table_name} where #{TimeEntry.table_name}.issue_id = #{Issue.table_name}.id)"
							) unless columns.detect { |c| c.name == :spent_time }
